...
|---------|--------|---------|
| Фреймворки с tool→component | Разработчик пишет компоненты, AI выбирает | Level 1 — Controlled |
| Декларативные JSON UI | AI генерит JSON, клиент рендерит из каталога | Level 2 — Declarative |
| **Cloud AI UI generators** | LLM пишет HTML/CSS/JS с нуля на каждый промпт | **Level 3 — Fully Generated** |
| **AI sandbox environments** | LLM генерит React/HTML приложения в sandbox | **Level 3 — Fully Generated** |
...
Никто из конкурентов не делает рефлексию над сгенерированным UI. Existing solutions generate and forget. **Overhuman запоминает и учится.**
...
|-------|--------|-------------------|
| "Generative UI: LLMs are Effective UI Generators" | Leviathan, Valevski (Nov 2025) | 44% на уровне человека-эксперта, dataset PAGEN |
...
| Протокол | Назначение |
|----------|-----------|
| MCP Apps (SEP-1865) | Интерактивный UI через sandboxed iframe в MCP |
| Declarative JSON UI protocols | Декларативный JSON (Level 2) |
| Agent-frontend transport layers | Транспортный слой agent-frontend |
| JSON UI schemas | JSON UI schema |

### Фреймворки

Существующие open-source фреймворки для Generative UI работают на Level 1-2 (controlled/declarative) или Level 3 (fully generated), но все ограничены web-only (React/Next.js/Python) и не поддерживают мультиустройства, self-healing, или UI-рефлексию. Ни один не написан на Go и не интегрирован с self-evolving agent pipeline.
...
|---|--------|---------|-------------|
| 1 | Уровень генерации | **Hybrid Level 2+3** | Level 2 (declarative fast path) для типовых паттернов (таблица, код, список, KV, JSON, ошибка, короткий текст) — 0 токенов, instant; Level 3 (LLM) для сложного/нестандартного контента |
...
| Что | Существующие решения | Overhuman |
|-----|---------------------|-----------|
| Генерация UI | Level 2 OR Level 3 | **✅ Hybrid 2+3: fast path + LLM fallback** |
| Мультиустройства | Нет (только web) | **✅ CLI + Web + Tablet kiosk** |
| Self-Healing UI | Нет (галлюцинация = ошибка юзеру) | **✅ Auto-retry + fallback** |
| Thought Logs | Частично (streaming в некоторых SDK) | **✅ Полная цепочка pipeline stages** |
| Progressive Disclosure | Нет (всё или ничего) | **✅ TL;DR + drill-down** |
| Canvas layout | AI sandbox environments (простой) | **✅ Sidebar + Canvas + Dynamic Expand** |
...
| Стоимость | Дорого (мощная модель) | **✅ Fast path: 0¢; LLM: 0.1¢ (gpt-4.1-nano)** |
| Self-evolving | Нет | **✅ UI эволюционирует с агентом** |

**Overhuman = первый self-evolving AI agent с hybrid Level 2+3 multi-device UI, self-healing, progressive disclosure + UI рефлексией.**

---

## 21. HYBRID LEVEL 2+3: FAST PATH (март 2026)

### Мотивация

Индустрия массово выбрала Level 2 (declarative JSON) для типовых визуализаций: надёжнее, быстрее, дешевле. Level 3 (LLM-generated) нужен только для нестандартного контента. Overhuman теперь использует **оба уровня**: fast path для ~90% ответов (0 токенов, 0мс, 0¢), LLM generation для ~10% сложных случаев.

### Архитектура

```
Pipeline.Run() → RunResult
        ↓
  TryFastPath(result, format)
        ↓
  Matched? ─── Да ──→ Declarative render (instant, 0 tokens)
        │
        Нет
        ↓
  LLM Generate (Level 3, ~2s, ~0.1¢)
```

### Детекция контента (приоритет)

| # | Тип | Детекция | Пример |
|---|-----|----------|--------|
| 1 | JSON | `json.Valid()` + начинается с `{`/`[` | `{"name":"Alice"}` |
| 2 | Error | Ключевые слова: `error:`, `failed:`, `panic:`, stack traces | `Error: connection refused` |
| 3 | Table | `\|`-separated или tab-separated, >= 2 строки данных | `\| Name \| Age \|` |
| 4 | Code | Triple backtick fence, или индентация + >= 3 code keywords | `` ```go\nfunc main()... `` |
| 5 | List | >= 2 строки с `- `, `* `, `• `, `1. ` | `- First\n- Second` |
| 6 | Key-Value | >= 50% строк содержат `key: value` или `key = value` | `host: localhost` |
| 7 | Short | < 200 символов, без структуры | `Hello world` |
| 8 | Unknown | Не совпал ни один паттерн → **LLM fallback** | Длинный прозаический текст |

### Рендеринг

Каждый тип контента имеет два рендерера: **ANSI** (CLI) и **HTML** (web/tablet).

- ANSI: box drawing (┌┐└┘│─├┤┬┴┼), ANSI цвета (cyan, red, grey), bold headers
- HTML: полная HTML страница с dark theme (#1a1a2e, #e0e0e0, #00d4aa), inline CSS, zero external deps

FormatReact и FormatMarkdown **всегда** проходят через LLM (React требует компонентной генерации).

### Файл: `internal/genui/fastpath.go`

```go
// TryFastPath — точка входа. Возвращает Matched=true если контент отрендерен декларативно.
func TryFastPath(content string, format UIFormat) FastPathResult

// ContentType: table, code, list, key_value, json, error, short, unknown
// Приоритет детекции: JSON > Error > Table > Code > List > KV > Short > Unknown
```

### Интеграция в UIGenerator
...
func (g *UIGenerator) Generate(ctx context.Context, result pipeline.RunResult, caps DeviceCapabilities) (*GeneratedUI, error) {
    ...
    if g.fastPathEnabled {
        if fp := TryFastPath(result.Result, format); fp.Matched {
            return &GeneratedUI{TaskID: result.TaskID, Format: format, Code: fp.Code, Source: "fastpath"}, nil
        }
    }

    // Level 3: LLM generation (existing behavior)
    ...
}
```
...
`GeneratedUI.Source` = `"fastpath"` или `"llm"` — для рефлексии, A/B тестирования и метрик.
